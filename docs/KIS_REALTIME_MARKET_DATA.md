...
- **백오프 리셋 기준은 소켓 open이 아니라 첫 구독 성공(`rt_cd === "0"`)** — KIS는 핸드셰이크를 통과시킨 뒤 구독 응답에서 거절하므로, open에서 리셋하면 거절 루프가 최소 지연으로 무한 반복된다
- 재연결 후 현재 추천 후보를 다시 구독
- 실시간 호가와 체결의 수신 시각을 각각 추적
- 승인키는 최대 23시간 동안 메모리에서 재사용
- 동일 에러 연속 반복은 기본 60초 간격으로 접어 기록하며 `repeatCount`로 실제 횟수를 남긴다(`errorRepeatIntervalMs`)

## App Key 동시 세션 제약

KIS WebSocket은 App Key당 세션을 하나만 허용한다. 다른 세션이 점유 중이면 핸드셰이크는 통과하고 구독 응답이 `ALREADY IN USE appkey`로 거절된다. 재시도로는 풀리지 않는다.

이 거절을 받으면 상태를 `BLOCKED`으로 두고 재연결을 중단하며, `status()`의 `blocked`와 `blockedReason`으로 원인을 노출한다. 점유 세션을 정리한 뒤 `clearBlock()`으로 재시도한다.

### 2026-08-04 사고 기록

시세용과 별개 세션이 같은 App Key를 점유한 상태에서 기록 세션이 시작됐고, 위 두 결함이 겹쳐 다음이 발생했다.

- `ALREADY IN USE appkey` 거절 13,045건, 소켓 오류 13,057건
- 재연결 13,057회, 중위 간격 1,046ms (설계상 30초까지 늘어야 했음)
- 225분간 지속, 저널 120MB, 실시간 시세 수신 **0건**
- 해당 세션의 기록 데이터는 리플레이·성과 검증에 사용할 수 없음

시세용 App Key와 주문용 App Key를 분리하면 이 충돌 자체가 줄어든다(`docs/KIS_LIVE_TRADING.md`의 자격정보 공유 항목 참고).
