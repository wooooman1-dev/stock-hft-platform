...
export class KisLiveReconciliationError extends Error {
  constructor(message, {
    code = "KIS_LIVE_RECONCILIATION_ERROR",
    statusCode = 409,
  } = {}) {
    super(message);
    this.name = "KisLiveReconciliationError";
    this.code = code;
    this.statusCode = statusCode;
  }
}

export class KisLiveReconciler {
  ...
  acknowledge() {
    ...
    if (!report || report.status !== "CONSISTENT") {
      throw new KisLiveReconciliationError(
        "KIS 주문내역·잔고 불일치가 해소되지 않아 주문 차단을 해제할 수 없습니다.",
        { code: "KIS_LIVE_RECONCILIATION_UNRESOLVED", statusCode: 409 },
      );
    }
