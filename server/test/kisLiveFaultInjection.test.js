...
// 이 파일은 실제 HTTP 계층(KisLiveTradingClient의 fetch/재시도/JSON 파싱)에서 발생하는
// 네트워크 타임아웃, 5xx, 잘못된 JSON, 명확한 거절 응답을 주입해 KisLiveOrderService의
// UNKNOWN_RESULT 처리·킬 스위치·재시작 멱등 복원이 실제로 각 실패 유형에서 올바르게
// 동작하는지 종단 간(end-to-end)으로 검증한다. 모의투자용 kisPaperFaultInjection.test.js와
// 동일한 구조이며, 실제 KIS 실전 엔드포인트에는 절대 요청하지 않는다.
...
function faultInjectionClient({
  ...
  return new KisLiveTradingClient({
    ...
    tokenStore: {
      loadValid() { return { accessToken: "live-token", issuedAt: 1, expiresAt: NOW + 60_000 }; },
...
function service({ client, journal = new MemoryJournal(), onUnknownResult = () => {} } = {}) {
  return new KisLiveOrderService({
    client,
    journal,
    limits: { maxOrderQuantity: 1, maxOrderValue: 10_000_000, maxDailyOrders: 20, maxDailyLoss: 0, maxConsecutiveLosses: 0 },
    now: () => NOW,
...
test("a timeout during submit produces UNKNOWN_RESULT, trips the kill switch, and survives a restart", async () => {
  ...
  await assert.rejects(
    ...
    (error) => error.statusCode === 423
      && (error.code === "KIS_LIVE_KILL_SWITCH" || error.code === "KIS_LIVE_RECONCILIATION_BLOCKED"),
  );
