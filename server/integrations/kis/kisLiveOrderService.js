...
import { KisLiveReconciler } from "../../domain/kisLiveReconciler.js";
import { KisLivePerformanceTracker } from "../../domain/kisLivePerformanceTracker.js";
import { computeFillModelComparison } from "../../domain/fillModelComparison.js";
import {
  KisLiveOrderServiceError,
  finiteNumberOrNull,
  koreaDateKey,
  ...
  safeRequest,
  unknownResponse,
} from "./kisLiveOrderSupport.js";

export { KisLiveOrderServiceError } from "./kisLiveOrderSupport.js";
...
// 카나리 단계 하드 상한: kisLiveConfig.js의 설정 로드 시점 검증(maximum=1)과는 별개로,
// 런타임에서도 수량이 정확히 1주가 아니면 거부한다. 둘 중 하나가 나중에 실수로 완화되어도
// 다른 하나가 실전 주문 규모를 계속 1주로 제한한다.
const CANARY_MAX_ORDER_QUANTITY = 1;

export class KisLiveOrderService {
  constructor({
    client,
    journal,
    limits,
    ...
    if (!limits) throw new TypeError("KIS live 안전 한도가 필요합니다.");
  ...
  setKillSwitch(enabled) {
    if (!enabled && this.unknownResult) {
      throw new KisLiveOrderServiceError(
        ...
        { code: "KIS_LIVE_UNKNOWN_RESULT_UNRESOLVED", statusCode: 409 },
      );
    }
  ...
  async execute({ operation, clientOrderId, request, call, orderBookSnapshot = null }) {
    ...
    try {
      ...
      try {
        ...
        throw new KisLiveOrderServiceError(
          "실전주문은 증권사에서 접수됐지만 실행 저널 기록에 실패했습니다. 중복 주문 방지를 위해 주문 결과 불명 상태로 전환했습니다.",
          { code: "KIS_LIVE_RESULT_JOURNAL_FAILED", statusCode: 500, ambiguous: true },
        );
      }
      ...
      if (error instanceof KisLiveOrderServiceError && error.code === "KIS_LIVE_RESULT_JOURNAL_FAILED") {
        throw error;
      }
  ...
  async enforceSafety(operation, request) {
    ...
    if (!Number.isFinite(riskPrice) || riskPrice <= 0) {
      throw new KisLiveOrderServiceError(
        ...
        { code: "KIS_LIVE_REFERENCE_PRICE_REQUIRED", statusCode: 400 },
      );
    }
    ...
    if (orderValue > this.limits.maxOrderValue) {
      throw new KisLiveOrderServiceError(
        ...
        { code: "KIS_LIVE_ORDER_VALUE_LIMIT", statusCode: 400 },
      );
    }
    const today = koreaDateKey(this.now());
    ...
    if (todayCount >= this.limits.maxDailyOrders) {
      throw new KisLiveOrderServiceError(
        ...
        { code: "KIS_LIVE_DAILY_ORDER_LIMIT", statusCode: 429 },
      );
    }
  ...
  async executeUnknownResolution(input) {
    ...
    if (!UNKNOWN_RESOLUTIONS.has(resolution)) {
      throw new KisLiveOrderServiceError(
        ...
        { code: "KIS_LIVE_UNKNOWN_RESOLUTION_INVALID", statusCode: 400 },
      );
    }
    ...
    if (!state) {
      throw new KisLiveOrderServiceError(
        ...
        { code: "KIS_LIVE_UNKNOWN_COMMAND_NOT_FOUND", statusCode: 404 },
      );
    }
    if (state.state !== "UNKNOWN") {
      throw new KisLiveOrderServiceError(
        `clientOrderId ${clientOrderId} 주문은 결과 불명 상태가 아니어서 해소할 수 없습니다.`,
        { code: "KIS_LIVE_COMMAND_NOT_UNKNOWN", statusCode: 409 },
      );
    }
    ...
    try {
      ...
      throw new KisLiveOrderServiceError(
        ...
        { code: "KIS_LIVE_UNKNOWN_RESOLUTION_JOURNAL_FAILED", statusCode: 500 },
      );
    }
    Object.assign(state, {
      state: "RESULT",
  ...
  async loadUnknownResolutionEvidence() {
    if (typeof this.client.getDailyOrders !== "function") {
      throw new KisLiveOrderServiceError(
        ...
        { code: "KIS_LIVE_UNKNOWN_RESOLUTION_EVIDENCE_UNAVAILABLE", statusCode: 503 },
      );
    }
    let history = null;
    try {
      ...
      throw new KisLiveOrderServiceError(
            ...
            : "실행 저널에 주문 결과를 기록하지 못했습니다.",
        {
          code: type === COMMAND_EVENT
            ? "KIS_LIVE_COMMAND_JOURNAL_FAILED"
            : baselineFailure
              ? "KIS_LIVE_RISK_BASELINE_JOURNAL_FAILED"
              : "KIS_LIVE_RESULT_JOURNAL_FAILED",
          statusCode: 500,
  ...
  markInterruptedCommandsUnknown() {
    for (const state of this.commands.values()) {
      ...
      const payload = {
        ...
        error: {
          code: "KIS_LIVE_INTERRUPTED_COMMAND",
...
function matchAcceptedBrokerOrder({ state, input, history, tracked }) {
  ...
  if (!orderNumber) {
    throw new KisLiveOrderServiceError(
      ...
      { code: "KIS_LIVE_UNKNOWN_RESOLUTION_ORDER_NUMBER_REQUIRED", statusCode: 400 },
    );
  }
  ...
  if (!matched) {
    throw new KisLiveOrderServiceError(
      ...
      { code: "KIS_LIVE_UNKNOWN_RESOLUTION_ORDER_NOT_FOUND", statusCode: 409 },
    );
  }
  if (tracked.has(orderNumber)) {
    throw new KisLiveOrderServiceError(
      `주문번호 ${orderNumber}는 이미 다른 실행 저널 명령의 결과로 기록돼 있습니다.`,
      { code: "KIS_LIVE_UNKNOWN_RESOLUTION_ORDER_ALREADY_TRACKED", statusCode: 409 },
    );
  }
  ...
  if (conflicts.length > 0) {
    throw new KisLiveOrderServiceError(
      `주문번호 ${orderNumber}가 실행 저널 명령과 일치하지 않습니다: ${conflicts
        ...
        .join(", ")}`,
      { code: "KIS_LIVE_UNKNOWN_RESOLUTION_ORDER_CONFLICT", statusCode: 409 },
    );
  }
  return matched;
}
...
function assertNoBrokerCandidate({ state, history, tracked }) {
  ...
  if (candidates.length > 0) {
    throw new KisLiveOrderServiceError(
      ...
      { code: "KIS_LIVE_UNKNOWN_RESOLUTION_CANDIDATE_EXISTS", statusCode: 409 },
    );
  }
  return null;
}
...
function unknownResolutionResponse({ state, resolution, matchedOrder, now }) {
  ...
  if (resolution === "NOT_ACCEPTED") {
    return {
      ...
      replayed: false,
      error: {
        code: "KIS_LIVE_UNKNOWN_RESOLVED_NOT_ACCEPTED",
  ...
  return {
    ...
    result: {
      ...
      mode: matchedOrder?.mode ?? "LIVE_TRADING",
      environment: matchedOrder?.environment ?? "LIVE",
      status: "ACCEPTED",
...
function optionalNote(value) {
  ...
  if (note.length > 500) {
    throw new KisLiveOrderServiceError("대조 메모는 500자 이하여야 합니다.", {
      code: "KIS_LIVE_UNKNOWN_RESOLUTION_NOTE_TOO_LONG",
      statusCode: 400,
    });
  }
  return note;
...
function reconciliationErrorCode(reconciliation) {
  ...
  if (status === "MISMATCH" || status === "RESOLVED_AWAITING_ACK") {
    return "KIS_LIVE_RECONCILIATION_BLOCKED";
  }
  if (status === "PENDING") return "KIS_LIVE_RECONCILIATION_PENDING";
  return "KIS_LIVE_RECONCILIATION_UNAVAILABLE";
}
