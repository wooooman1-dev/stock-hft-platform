...
import {
  ...
  redactText,
  textOrEmpty,
} from "./kisLiveTradingSupport.js";

export { KisLiveApiError } from "./kisLiveTradingSupport.js";
...
// 실전(TTTC*) TR ID는 한국투자 공식 GitHub(koreainvestment/open-trading-api)의
// examples_llm/domestic_stock/{inquire_balance,order_cash,order_rvsecncl,inquire_daily_ccld}에서
// env_dv === "real" 분기 값을 직접 확인해 채웠다(모의 VTTC* 접두사를 추정으로 바꾼 것이 아님).
// inquire_daily_ccld는 3개월 이내(TTTC0081R)/이전(CTSC9215R) 두 TR ID가 있으나, 이 클라이언트는
// 모의투자 클라이언트와 동일하게 항상 당일 조회만 사용하므로 3개월 이내 TR ID만 구현한다.
const BALANCE_TR_ID = "TTTC8434R";
const BUY_TR_ID = "TTTC0012U";
const SELL_TR_ID = "TTTC0011U";
const REVISE_CANCEL_TR_ID = "TTTC0013U";
const DAILY_ORDERS_TR_ID = "TTTC0081R";
...
export class KisLiveTradingClient {
  constructor({
    config,
    tokenStore,
    ...
    if (config?.mode !== KIS_LIVE_MODE_TRADING || !config.enabled) {
      throw new TypeError("KIS LIVE_TRADING 설정이 필요합니다.");
    }
    if (!tokenStore || typeof tokenStore.loadValid !== "function" || typeof tokenStore.save !== "function") {
      throw new TypeError("유효한 KIS live tokenStore가 필요합니다.");
    }
  ...
  status() {
    return {
      ...publicKisLiveConfiguration(this.config),
      token: this.tokenStore.status(),
      balanceApiAvailable: true,
      orderApiAvailable: Boolean(this.config.orderEnabled),
  ...
  async getBalance() {
    ...
    for (let page = 0; page < 10; page += 1) {
      ...
      assertSuccessfulPayload("잔고 조회", response, payload, {
        code: "KIS_LIVE_BALANCE_REJECTED",
  ...
  async fetchDailyOrders(query) {
    ...
    for (let page = 0; page < 10; page += 1) {
      ...
      assertSuccessfulPayload("주문·체결내역 조회", response, payload, {
        code: "KIS_LIVE_ORDER_HISTORY_REJECTED",
        secrets: this.secrets(),
      });
      pages.push(payload);
      const continuation = continuationHeader(response);
  ...
  async submitOrder(input) {
    if (!this.config.orderEnabled) {
      throw new KisLiveApiError(
        "한국투자 실전투자 주문 제출이 비활성화되어 있습니다(PULSEHFT_KIS_LIVE_ORDER_ENABLED).",
        { code: "KIS_LIVE_ORDER_SUBMISSION_DISABLED", statusCode: 423, ambiguous: false, operation: "SUBMIT" },
      );
    }
    ...
    const { response, payload } = await this.authorizedRequest("실전주문", new URL(ORDER_CASH_PATH, this.config.baseUrl), {
      method: "POST",
      trId,
      body,
      ambiguousOnFailure: true,
    });
    assertSuccessfulPayload("실전주문", response, payload, {
      code: "KIS_LIVE_ORDER_REJECTED",
      secrets: this.secrets(),
  ...
  async reviseOrder(input) {
    if (!this.config.orderEnabled) {
      throw new KisLiveApiError(
        "한국투자 실전투자 주문 제출이 비활성화되어 있습니다(PULSEHFT_KIS_LIVE_ORDER_ENABLED).",
        { code: "KIS_LIVE_ORDER_SUBMISSION_DISABLED", statusCode: 423, ambiguous: false, operation: "REVISE" },
      );
    }
  ...
  async assertCancelable(request) {
    ...
    if (!match) {
      throw new KisLiveApiError("한국투자 실전계좌에서 정정·취소 가능한 원주문을 찾을 수 없습니다.", {
        code: "KIS_LIVE_ORDER_NOT_CANCELABLE",
        statusCode: 409,
        ambiguous: false,
    ...
    if (request.quantity > match.cancelableQuantity) {
      throw new KisLiveApiError(`정정·취소 수량은 가능 수량 ${match.cancelableQuantity}주를 초과할 수 없습니다.`, {
        code: "KIS_LIVE_CANCEL_QUANTITY_EXCEEDED",
        statusCode: 409,
        ambiguous: false,
        operation: request.operation,
      });
  ...
  async executeReviseCancel(request, divisionCode) {
    ...
    const operationLabel = request.operation === "REVISE" ? "실전주문 정정" : "실전주문 취소";
    ...
    assertSuccessfulPayload(operationLabel, response, payload, {
      code: request.operation === "REVISE" ? "KIS_LIVE_REVISE_REJECTED" : "KIS_LIVE_CANCEL_REJECTED",
      secrets: this.secrets(),
      ambiguousHttp5xx: true,
    });
    this.invalidateDailyOrdersCache();
  ...
  async issueAccessToken() {
    ...
    if (!response.ok) {
      throw apiFailure("접근토큰 발급", response.status, payload, {
        code: "KIS_LIVE_TOKEN_HTTP_ERROR",
    ...
    if (!accessToken || !Number.isFinite(expiresInSeconds) || expiresInSeconds <= 0) {
      throw new KisLiveApiError("한국투자 실전투자 접근토큰 응답 형식이 올바르지 않습니다.", {
        code: "KIS_LIVE_TOKEN_INVALID_RESPONSE",
      });
    }
  ...
  async rawRequest(operation, url, options) {
    ...
    try {
      ...
      throw new KisLiveApiError(
        ...
        {
          code: timeoutFailure ? "KIS_LIVE_REQUEST_TIMEOUT" : "KIS_LIVE_NETWORK_ERROR",
          statusCode: 502,
