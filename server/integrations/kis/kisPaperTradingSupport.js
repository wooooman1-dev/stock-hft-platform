import { KIS_PAPER_MODE_TRADING } from "./kisPaperConfig.js";

const ALLOWED_EXCHANGES = new Set(["KRX", "NXT", "SOR"]);

export class KisPaperApiError extends Error {
  constructor(message, {
    code = "KIS_PAPER_API_ERROR",
    statusCode = 502,
    ambiguous = false,
    operation = null,
  } = {}) {
    super(message);
    this.name = "KisPaperApiError";
    this.code = code;
...
export function normalizeOrderResponse(output, context) {
  if (!isRecord(output)) {
    throw new KisPaperApiError("한국투자 모의주문 응답에 output 객체가 없습니다.", {
      code: "KIS_PAPER_ORDER_INVALID_RESPONSE",
      ambiguous: true,
  ...
  if (!orderNumber || !orderOrganizationNumber) {
    throw new KisPaperApiError("한국투자 모의주문 응답에 주문번호 또는 주문조직번호가 없습니다.", {
      code: "KIS_PAPER_ORDER_INVALID_RESPONSE",
      ambiguous: true,
      operation: context.operation,
    });
...
export async function parseJson(response, operation, ambiguous) {
  try {
    ...
    throw new KisPaperApiError(`한국투자 ${operation} 응답이 JSON이 아닙니다.`, {
      code: "KIS_PAPER_INVALID_JSON_RESPONSE",
      ambiguous,
      operation,
    });
  }
...
export function apiFailure(operation, status, payload, {
  ...
  return new KisPaperApiError(`한국투자 ${operation} 실패${message ? `:${message}` : ""}`, {
    code,
...
export function invalidInput(message) {
  return new KisPaperApiError(message, {
    code: "KIS_PAPER_INVALID_INPUT",
    statusCode: 400,
    ambiguous: false,
  });
}
