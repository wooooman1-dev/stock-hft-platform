...
searchStyle.textContent = String.raw`.instrument-search-trigger{display:inline-flex;align-items:center;gap:6px;height:30px;border:1px solid #2a3a51;border-radius:8px;background:#0d1624;color:#aebbd0;padding:0 10px;font-size:10px;font-weight:700;white-space:nowrap}.instrument-search-trigger:hover{border-color:#59cfe7;color:#dffaff}.instrument-search-trigger span{font-size:15px;line-height:1;color:#67dff2}.instrument-search-backdrop{position:fixed;z-index:40;inset:0;background:rgba(2,5,10,.72);backdrop-filter:blur(8px);display:grid;place-items:start center;padding-top:82px}.instrument-search-backdrop[hidden]{display:none}.instrument-search-open{overflow:hidden}.instrument-search-panel{width:min(680px,calc(100vw - 40px));max-height:calc(100vh - 118px);display:flex;flex-direction:column;border:1px solid #263750;border-radius:18px;background:linear-gradient(180deg,#101927,#090f19);box-shadow:0 30px 90px rgba(0,0,0,.58);overflow:hidden}.instrument-search-header{display:flex;justify-content:space-between;align-items:center;padding:18px 20px 12px}.instrument-search-header>div{display:flex;flex-direction:column;gap:3px}.instrument-search-header span{color:#647892;font-size:9px;font-weight:800;letter-spacing:.15em}.instrument-search-header h2{margin:0;font-size:20px}.instrument-search-close{width:34px;height:34px;border:1px solid #2a3a50;border-radius:9px;background:#0b1320;color:#8797ad;font-size:22px;line-height:1}.instrument-search-close:hover{color:#fff;border-color:#4a617d}.instrument-search-input-wrap{position:relative;display:block;margin:0 20px}.instrument-search-input-wrap input{width:100%;height:48px;border:1px solid #31445f;border-radius:11px;background:#070d16;color:#f0f6ff;padding:0 66px 0 15px;font-size:14px;outline:none}.instrument-search-input-wrap input:focus{border-color:#55d1e8;box-shadow:0 0 0 3px rgba(85,209,232,.1)}.instrument-search-input-wrap kbd{position:absolute;right:12px;top:50%;transform:translateY(-50%);border:1px solid #2b3a50;border-radius:5px;background:#111b2a;color:#708198;padding:3px 6px;font:9px/1.2 inherit}.instrument-search-status{padding:10px 22px 8px;color:#6f8096;font-size:10px}.instrument-search-results{min-height:120px;max-height:390px;overflow:auto;padding:0 12px 8px}.instrument-search-result{width:100%;display:grid;grid-template-columns:1fr 78px 150px;align-items:center;gap:10px;border:1px solid transparent;border-radius:9px;background:transparent;color:#dce7f5;padding:11px 10px;text-align:left}.instrument-search-result:hover,.instrument-search-result.active{background:#121e2d;border-color:#2b405a}.instrument-search-result:disabled{opacity:.6}.instrument-search-result-name{display:flex;align-items:center;gap:7px;font-size:13px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.instrument-search-current{border:1px solid rgba(89,216,238,.3);border-radius:999px;background:rgba(89,216,238,.08);color:#66d9ee;padding:3px 6px;font-size:8px;font-weight:800}.instrument-search-result-code{color:#85a1be;font-size:11px;font-variant-numeric:tabular-nums}.instrument-search-result-meta{text-align:right;color:#62748c;font-size:10px}.instrument-search-empty{display:grid;place-items:center;min-height:108px;color:#586a82;font-size:11px}.instrument-search-loading{display:flex;align-items:center;justify-content:center;gap:9px;min-height:108px;color:#70839a;font-size:11px}.instrument-search-loading i{width:14px;height:14px;border:2px solid #29405a;border-top-color:#62d7ed;border-radius:50%;animation:instrument-search-spin .8s linear infinite}@keyframes instrument-search-spin{to{transform:rotate(360deg)}}.instrument-search-error{margin:0 20px 12px;display:flex;flex-direction:column;gap:5px;border:1px solid rgba(255,103,136,.3);border-radius:10px;background:rgba(255,87,123,.06);padding:12px}.instrument-search-error[hidden]{display:none}.instrument-search-error strong{font-size:12px}.instrument-search-error span{color:#e78ba0;font-size:10px;line-height:1.55}.instrument-search-boundary{margin:0;padding:10px 20px 16px;color:#52647a;font-size:9px;line-height:1.55;border-top:1px solid #162234}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}@media(max-width:720px){.instrument-search-backdrop{padding-top:20px}.instrument-search-panel{max-height:calc(100vh - 40px)}.instrument-search-result{grid-template-columns:1fr 72px}.instrument-search-result-meta{display:none}}`;
...
overlay.innerHTML = `
  <section class="instrument-search-panel" role="dialog" aria-modal="true" aria-labelledby="instrument-search-title">
    <header class="instrument-search-header">
      <div>
        <span>KIS DOMESTIC INSTRUMENTS</span>
        <h2 id="instrument-search-title">종목 선택</h2>
      </div>
    ...
    <p class="instrument-search-boundary">선택한 종목의 KIS 현재가와 호가단위를 기준으로 메인 SIMULATION 차트·호가·체결·분석을 새로 시작합니다. 내부 모의주문, 보유수량, 주문내역 또는 자동전략이 남아 있으면 안전을 위해 전환이 차단됩니다. 한국투자 모의계좌 주문은 변경하지 않습니다.</p>
  </section>`;
...
async function runSearch(query) {
  ...
  try {
    const response = await fetch(`${SEARCH_ENDPOINT}?q=${encodeURIComponent(query)}&limit=20`, {
    ...
    status.textContent = currentResults.length > 0
      ? `${currentResults.length}개 결과${suffix} · 선택하면 메인 종목이 변경됩니다.`
      : "일치하는 국내 종목이 없습니다.";
    renderResults();
  } catch (error) {
    if (requestId !== lastRequestId) return;
...
function renderResults() {
  ...
  resultsHost.innerHTML = currentResults.length > 0
    ? currentResults.map((instrument, index) => resultMarkup(
      instrument,
      index,
      index === activeIndex,
  ...
  const active = resultsHost.querySelector('[aria-selected="true"]');
...
function renderRecent() {
  ...
  resultsHost.innerHTML = currentResults.length > 0
    ? currentResults.map((instrument, index) => resultMarkup(
      ...
      index,
      index === activeIndex,
      instrument.symbol === getCurrentSymbol(),
    )).join("")
...
async function selectResult(index) {
  ...
  activeIndex = index;
  clearError();
  status.textContent = `${instrument.name}을(를) 메인 종목으로 전환하고 있습니다.`;
  resultsHost.innerHTML = `<div class="instrument-search-loading"><i></i><span>KIS 현재가·호가단위 확인 후 SIMULATION을 초기화합니다.</span></div>`;
  try {
...
function readRecent() {
  try {
    ...
    return parsed.filter(isInstrument).slice(0, MAX_RECENT);
  } catch {
    return [];
  }
}
...
function saveRecent(instrument) {
  const next = [instrument, ...readRecent().filter((item) => item.symbol !== instrument.symbol)]
    .slice(0, MAX_RECENT);
...
function showError(message) {
  ...
  errorHost.innerHTML = `<strong>전환 차단</strong><span>${escapeHtml(message)}</span>`;
}
