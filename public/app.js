...
function render() {
  ...
  app.innerHTML = `
    ...
    </section><footer><span>시세·호가·체결은 KIS 실전계좌 읽기 전용 데이터입니다.</span><span>이 화면의 잔고와 주문은 KIS 모의투자이며 자동전략과 연결하지 않습니다. 실전(실제 자금) 주문은 이중 안전플래그로 잠긴 별도 카나리 패널에서만 가능합니다.</span></footer>`;
