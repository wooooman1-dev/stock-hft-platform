...
const FIELD_IDS = Object.freeze({
  ...
  stopLossPercent: "strategy-stop-loss",
  takeProfitPercent: "strategy-take-profit",
  trailingStopPercent: "strategy-trailing-stop",
  maxHoldingSeconds: "strategy-max-holding-seconds",
});
...
export function settingsToDraft(value) {
  return {
    ...
    stopLossPercent: bpsToPercentString(value.stopLossBps),
    takeProfitPercent: bpsToPercentString(value.takeProfitBps),
    trailingStopPercent: bpsToPercentString(value.trailingStopBps),
    maxHoldingSeconds: value.maxHoldingMs === null ? "" : String(value.maxHoldingMs / 1_000),
  };
}
...
export function draftToPayload(value) {
  return {
    ...
    stopLossBps: parseOptionalPercentToBps(value.stopLossPercent, "손절률"),
    takeProfitBps: parseOptionalPercentToBps(value.takeProfitPercent, "익절률"),
    trailingStopBps: parseOptionalPercentToBps(value.trailingStopPercent, "트레일링 스톱"),
    maxHoldingMs: parseOptionalSecondsToMs(value.maxHoldingSeconds, "최대 보유시간"),
  };
}
...
export function describeAutoStrategy(value) {
  ...
  return `진입 ${value.entryMinimumConfidence}% · 청산 ${value.exitMinimumConfidence}% · 최대 ${value.maximumSpreadTicks}틱 · ${value.orderQuantity}주 · ${value.cooldownMs / 1_000}초 대기 · 위험청산 ${riskCount}개`;
}

export function describeRiskExitSettings(value) {
  if (!value) return "위험청산 설정을 불러오는 중";
  return [
    `손절 ${formatPercentSetting(value.stopLossBps)}`,
    `익절 ${formatPercentSetting(value.takeProfitBps)}`,
    `트레일링 ${formatPercentSetting(value.trailingStopBps)}`,
    `최대보유 ${value.maxHoldingMs === null ? "OFF" : `${value.maxHoldingMs / 1_000}초`}`,
  ].join(" · ");
}
...
function formatPercentSetting(value) {
  return value === null ? "OFF" : `${value / 100}%`;
}
...
function parseOptionalPercentToBps(value, label) {
  const text = String(value ?? "").trim();
  if (text === "") return null;
  if (!/^\d+(?:\.\d{1,2})?$/.test(text)) {
    throw new Error(`${label}은 소수점 둘째 자리까지 입력할 수 있습니다.`);
  }
  ...
  if (!Number.isInteger(basisPoints) || basisPoints < 1 || basisPoints > 10_000) {
    throw new Error(`${label}은 0.01% 이상 100% 이하로 입력해야 합니다.`);
  }
  return basisPoints;
}

function parseOptionalSecondsToMs(value, label) {
  const text = String(value ?? "").trim();
  if (text === "") return null;
  if (!/^\d+$/.test(text)) throw new Error(`${label}은 1초 이상의 정수여야 합니다.`);
  const seconds = Number(text);
  const milliseconds = seconds * 1_000;
  if (!Number.isSafeInteger(milliseconds) || seconds < 1) {
    throw new Error(`${label}은 1초 이상의 안전한 정수여야 합니다.`);
  }
  return milliseconds;
}
...
function panelHtml() {
  ...
  return `<section class="strategy-settings-panel" data-strategy-settings-panel>
    <div class="strategy-settings-heading">
      <div><span>자동전략 설정</span><strong>신호 조건·진입 수량·위험청산</strong></div>
    ...
    <div class="strategy-settings-grid">
      ${field("진입 신뢰도", FIELD_IDS.entryMinimumConfidence, draft.entryMinimumConfidence, { min: 0, max: 100, step: 1, suffix: "%" })}
      ${field("청산 신뢰도", FIELD_IDS.exitMinimumConfidence, draft.exitMinimumConfidence, { min: 0, max: 100, step: 1, suffix: "%" })}
      ${field("최대 스프레드", FIELD_IDS.maximumSpreadTicks, draft.maximumSpreadTicks, { min: 1, max: 20, step: 1, suffix: "틱" })}
      ${field("진입 수량", FIELD_IDS.orderQuantity, draft.orderQuantity, { min: 1, max: 100, step: 1, suffix: "주" })}
      ${field("재진입 대기", FIELD_IDS.cooldownSeconds, draft.cooldownSeconds, { min: 1, max: 600, step: 1, suffix: "초", full: true })}
      <div class="strategy-settings-subheading">위험청산 · 빈칸은 사용 안 함</div>
      ${field("손절률", FIELD_IDS.stopLossPercent, draft.stopLossPercent, { min: 0.01, max: 100, step: 0.01, suffix: "%" })}
      ${field("익절률", FIELD_IDS.takeProfitPercent, draft.takeProfitPercent, { min: 0.01, max: 100, step: 0.01, suffix: "%" })}
      ${field("트레일링", FIELD_IDS.trailingStopPercent, draft.trailingStopPercent, { min: 0.01, max: 100, step: 0.01, suffix: "%" })}
      ${field("최대 보유", FIELD_IDS.maxHoldingSeconds, draft.maxHoldingSeconds, { min: 1, step: 1, suffix: "초" })}
    </div>
    <p class="strategy-settings-risk-summary">${describeRiskExitSettings(draftToSettingsPreview(draft))}</p>
    <p class="strategy-settings-status ${statusTone}">${statusMessage || "설정만 저장되며 자동전략은 서버 재시작 시 항상 꺼진 상태로 시작합니다."}</p>
  </section>`;
}

function draftToSettingsPreview(value) {
  try {
    return draftToPayload(value);
  } catch {
    return {
      stopLossBps: null,
      takeProfitBps: null,
...
function field(label, id, value, { min, max, step, suffix, full = false }) {
  const maxAttribute = max === undefined ? "" : ` max="${max}"`;
  return `<label class="${full ? "full-row" : ""}"><span>${label}</span><div><input id="${id}" type="number" min="${min}"${maxAttribute} step="${step}" value="${value}"><small>${suffix}</small></div></label>`;
}
