...
function render() {
  ...
  const realtimeStatus = status.dataSources?.realtime ?? {};
  const realtimeCounts = payload?.realtimeStateCounts ?? {};
  const realtimeConnected = Boolean(status.dataSources?.kis?.realtimeConfirmationAvailable);
  overlay.querySelector(".recommendation-panel").innerHTML = `
    <header class="recommendation-header">
      <div class="recommendation-title"><span>MARKET CANDIDATE SCANNER</span><h2>매수추천 리스트</h2><p>거래대금·등락률·체결강도 순위에서 후보를 찾고, 실시간 호가·체결로 진입 가능 상태를 추가 확인합니다.</p></div>
    ...
    <div class="recommendation-summary">
      ...
      ${summary("실시간 확정", realtimeConnected ? `${fmt(realtimeCounts.ENTRY_READY)} 준비` : "미연결", realtimeConnected ? `${fmt(realtimeStatus.desiredSymbolCount)}종목 감시` : "WebSocket 연결 필요")}
      ${summary("보조 데이터", auxiliarySourceLabel(status.dataSources), "DART·뉴스·커뮤니티")}
    </div>
    <div class="recommendation-source-warning">현재 목록은 KIS REST 후보를 WebSocket 호가·체결로 추가 확인합니다. ${auxiliarySourceDescription(status.dataSources)} ENTRY_READY도 분석 상태일 뿐 자동매수 신호가 아니며, 수수료·세금 값은 계좌별 실제 정산과 대사해야 합니다.</div>
...
function filteredCandidates() {
  ...
  if (activeFilter === "BLOCKED") {
    return candidates.filter((item) => item.stage === "BLOCKED" || item.realtime?.state === "BLOCKED");
  }
...
function row(item) {
  ...
  const realtimeState = item.realtime?.state ?? "SCANNED";
  const realtimeReasons = Array.isArray(item.realtime?.reasons) ? item.realtime.reasons : [];
  const restReason = item.stage === "BLOCKED"
    ? (item.blockReasons ?? []).join(" · ")
    : (item.reasons ?? []).slice(0, 2).join(" · ");
  const reason = realtimeReasons.slice(0, 2).join(" · ") || restReason;
  ...
  const realtimeMetrics = item.realtime?.metrics ?? {};
  return `<div class="recommendation-row">
    <span class="recommendation-rank">${fmt(item.rank)}</span>
    <span class="recommendation-stage stage-${String(realtimeState).toLowerCase().replaceAll("_", "-")}">${realtimeStateLabel(realtimeState)}</span>
    ...
    <strong>${formatPrice(realtimeMetrics.currentPrice ?? item.currentPrice)}</strong>
    ...
    <span class="recommendation-reasons"><b>${escapeHtml(reason || "근거 데이터 부족")}</b>실시간 스프레드 ${fmtDecimal(realtimeMetrics.spreadBps, 1)}bp · 호가 불균형 ${signedPercent(realtimeMetrics.bookImbalance)} · 체결강도 ${fmtDecimal(realtimeMetrics.executionStrength, 1)} · 데이터 ${fmt(item.dataCompleteness?.percent)}% · 공시 ${fmt(disclosureCount)} · 뉴스 ${fmt(newsCount)} · 커뮤니티 ${fmt(communityCount)}${latestNews ? `<br>${escapeHtml(latestNews)}` : ""}</span>
...
function realtimeStateLabel(value) {
  return ({
    SCANNED: "스캔",
    WATCH: "감시",
    REALTIME_CONFIRMING: "확인 중",
    ENTRY_READY: "진입 준비",
    BLOCKED: "차단",
    STALE: "지연",
    DISCONNECTED: "단절",
...
function signedPercent(value) { const number = Number(value); return Number.isFinite(number) ? `${number >= 0 ? "+" : ""}${(number * 100).toFixed(1)}%` : "-"; }
